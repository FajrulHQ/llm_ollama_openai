...
def save_chunks(chunks, output_folder):
    """Save chunks to files with consistent formatting."""
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
        
    for i, chunk in enumerate(chunks, 1):
        # Ensure chunk ends with single newline
        chunk = chunk.strip() + "\n"
        filename = f"{output_folder}/chunk_{i:03d}.txt"
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(chunk)

def main():
    pdf_path = "C:/Users/User/pdf_chunking_project/Draft_TKO.pdf"
    output_folder = "output_chunks_coba"
    ...
    print(f"Chunks telah disimpan di folder: {output_folder}")
